...
TEST(utils, CalculatingMeanTest) {
    ...
    std::cout << std::setprecision(3) << Rand::Utils::CalculateSequenceMean(sequence);
}

TEST(utils, CalculatingDevianceTest) {
    auto sequence = Random::GenerateSequence(12, 5, 9, 28);
    auto mean = Rand::Utils::CalculateSequenceMean(sequence);
    std::cout << std::setprecision(3) << Rand::Utils::CalculateSequenceDeviation(sequence, mean);
}

TEST(utils, CalculatingVarianceTest) {
    auto sequence = Random::GenerateSequence(12, 5, 9, 28);
    auto mean = Rand::Utils::CalculateSequenceMean(sequence);
    std::cout << std::setprecision(3) << Rand::Utils::CalculateSequenceVariance(sequence, mean);
}

TEST(utils, CalculatingImplicitCriteriaTest) {
    auto sequence = Random::GenerateSequence(12, 5, 9, 28);
    std::cout
        << "Actual: " << std::setprecision(10) << Rand::Utils::CalculateSequenceImplicitCriteria(sequence)
        << std::endl
...
TEST(utils, CalculatingAperiodicIntervalTest) {
    ...
    std::cout << Rand::Utils::CalculateSequenceAperiodicInterval(sequence, period);
}